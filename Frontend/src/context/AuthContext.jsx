...
export const AuthProvider=({children})=>{
    ...
    useEffect(() => {
        const loadUser = async () => { 
            console.log("AuthContext: loadUser started. Current path:", location.pathname);
            const showLoader = !publicPaths.includes(location.pathname) || location.pathname.startsWith('/api/auth/google/callback');
            if (!showLoader && !user) { 
                setLoading(false);
                console.log("AuthContext: Public path, no immediate loader. Attempting background auth check.");
            } else {
                 setLoading(true); 
            }
